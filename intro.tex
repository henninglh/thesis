\part{Introduction}
\label{pa:intro}
\section{The basic principles of a biomarker}
...
\section{The goals of this assignment}
The goals of this assignment is divided into two parts. The first part is
to develop Ranklust, a contribution to the Cytoscape plugin, clusterMaker2.
clusterMaker2 have numerous cluster algorithms and Ranklust implements ranking
algorithms to rank the clusters based on the users specifications. Secondly, to
assess its ability to prioritize network biomarkers in prostate cancer. The
second goal will be verified through two separate parts. The first part is to
benchmark and cross-calidate the ranked clusters. The second is to demonstrate
its application to rank possible clusters of biomarkers in case studies from
expert knowledge and high-throughput studies.

\chapter{Networks as a tool for biomarker discovery in cancer research}
\section{The limitations of existing biomarkers in prostate cancer}
...
\section{High-throughput technologies}
...
necessary in order to come to the next step in Ranklust, ranking the clusters.
...
\chapter{Ranklust: An tool to rank clusters of biomarkers based on their network properties}
\section{Cytoscape}
...
\section{Databases for gene information}
...
In this assignment, RefSeq IDs and UniProt IDs was used. This was also because
the protein identifiers from iRefWeb was listed in this exact format.
