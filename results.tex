...
contribution is already installed in Cytoscape.
\begin{figure}
    \label{fig:startup}
...
columns, a\_alias and b\_alias. The names for the columns does not have to be
exactly a\_alias and b\_alias, but there should be some header information
indicating a source and target gene/node. Having no header in the data imported
into cytoscape is not a problem, but it requires the user to explicitly specify
this in the "Advanced Options" menu and check off the "Use first line as column
names" option.
\begin{figure}[H]
    \label{fig:import}
...
The ranking panel relatinos (Figure: \ref{fig:rank-panel}) shows how the classes
